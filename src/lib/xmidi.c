 ...
 */
struct EventInfo {
    u8 * start; ///< Position in the MIDI stream where the event starts.
                  ///< For delta-based MIDI streams (e.g. SMF and XMIDI), this points to the delta.
    u32 delta; ///< The number of ticks after the previous event that this event should occur.
    u8 event; ///< Upper 4 bits are the command code, lower 4 bits are the MIDI channel.
                  ///< For META, event == 0xFF. For SysEx, event == 0xF0.
    union {
        struct {
            u8 param1; ///< The first parameter in a simple MIDI message.
            u8 param2; ///< The second parameter in a simple MIDI message.
        } basic;
        struct {
            u8   type; ///< For META events, this indicates the META type.
            u8 * data; ///< For META and SysEx events, this points to the start of the data.
        } ext;
    };
    u32 length; ///< For META and SysEx blocks, this indicates the length of the data.
...
struct XMIDI_info {
    u8 num_tracks;
    u8* tracks[120]; // Maximum 120 tracks
};
 ...
 */
static u16 read2low(u8** data);
static u32 read4high(u8** data);
static void write4high(u8** data, u32 val);
static void write2high(u8** data, u16 val);
static u32 readVLQ2(u8** data);
static u32 readVLQ(u8** data);
static i32 putVLQ(u8* dest, u32 value);
...
static struct EventInfo* pop_cached_event(u32 current_time, u32 delta);
static void save_event(struct EventInfo* info, u32 current_time);

static i32 read_event_info(u8* data, struct EventInfo* info, u32 current_time);
static i32 put_event(u8* dest, struct EventInfo* info);
static i32 convert_to_mtrk(u8* data, u32 size, u8* dest);
static i32 read_XMIDI_header(u8* data, u32 size, struct XMIDI_info* info);
...
{
    u8* d = *data;
    u16 value = (d[0] << 24) | (d[1] << 16) | (d[2] << 8) | (d[3]);
    *data = (d + 4);
    return value;
}
        ...
        if (!memcmp(pos, "XMID", 4)) {
            ...
            for (i = 4; i < len; i++) {
                ...
                if (memcmp(buf, "INFO", 4) == 0) {
                    ...
                    info->num_tracks = (u8)read2low(&pos);
                    pos += 2;
...
/********************************* Public API *********************************/
// Code adapted from the Exult engine
u32 convert_to_midi(u8* data, u32 size, u8** dest)
{
    i32 len;
    u8* d,* start;
